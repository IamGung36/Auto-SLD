...
const SidebarTableRow = ({ id, label, value, onChange, type = "text", suffix = "", highlightedField, setHighlightedField, inputRef, disabled = false }) => (
  <div className={`grid grid-cols-[100px_10px_1fr] items-center gap-2 border-b border-[#2d3748] last:border-0 py-2 px-3 transition-all duration-300 ${disabled ? 'opacity-30 pointer-events-none' : highlightedField === id ? 'bg-[#2a4365] -mx-1 shadow-inner border-l-4 border-l-cyan-400' : 'hover:bg-[#2d3748]'}`}>
    ...
    <div className="flex items-center gap-2">
      {type === "textarea" ? (
        ...
        <input disabled={disabled} ref={inputRef} value={value} onChange={(e) => onChange(e.target.value)} onFocus={() => setHighlightedField(id)} className="w-full bg-transparent text-[#e2e8f0] font-mono text-[13px] border-none focus:ring-0 outline-none p-0 placeholder-slate-600" />
      )}
...
const App = () => {
  ...
  const [globalSpecs, setGlobalSpecs] = useState({
    ...
    mdbIc: calculateMdbIc("2,500 kVA", "6%Zk", 400),
    mainCbType: "ACB 3P",
    mainCbAtAf: "4000/4000 AT/AF",
    mainPhaseCable: "11(4x1C-300) sq.mm., CV",
    mainGroundCable: "1C-120 sq.mm.,(G)",
    busbar: "4000A, 100%N, 25%G",    
    mainCal: {
       circType: 'Main', factor: 1.2, at: 4000, adjust: 1.0, af: 4000,
       install: "A: Ladder Closed", ref: "T5-33", cores: "1C", size: "300",
       ampCable: 468, ca: 1, cg: 1, set: 11, dist: 15, vd: 0, ground: "120", racewayOverride: ""
     }
   });
   ...
   const [feeders, setFeeders] = useState([
     ...
     { id: 4, title: "Feeder 4", type: "MCCB 3P", ataf: "250 AT / 250 AF", cable1: "4x1C-120 sq.mm., CV", cable2: "1C-25 sq.mm.,(G)", install: "E: Conduits UG", loadKw: "150", loadA: "216.51", loadTitle: "Inverter 4", cal: {...initialCal, circType: 'Feeder', at: 250, af: 250, install: "E: Conduits UG", ref: "T5-29", ampCable: 275, ground: "25"} },
   ]);
  ...
  const [dbSubTab, setDbSubTab] = useState('Google Sheet Sync'); 
  ...
  const fetchProjectsList = async (scriptUrl = appsScriptUrl) => {
    if (!scriptUrl || scriptUrl.trim() === '') return;
    try {
      const res = await fetch(`${scriptUrl}?action=getProjects`);
      const data = await res.json();
      if (Array.isArray(data)) {
        setSavedProjects(data);
        logSync(`Loaded ${data.length} saved projects from Google Sheet.`);
      }
    } catch (e) {
      logSync(`Failed to fetch project list: ${e.message}`);
    }
  };
  ...
  const handlePullDatabase = async (customSheetUrl = sheetUrl, customScriptUrl = appsScriptUrl) => {
    ...
    setSheetError('');
    logSync('Starting database sync...');
    ...
    try {
      if (customScriptUrl && customScriptUrl.trim() !== '') {
        logSync('Connecting to Google Apps Script Web App...');
        const res = await fetch(customScriptUrl);
        const data = await res.json();
        ...
        logSync(`Fetching public sheet CSV data (Spreadsheet ID: ${sheetId})...`);
        
        const fetchCSV = async (sheetName) => {
          const url = `https://docs.google.com/spreadsheets/d/${sheetId}/gviz/tq?tqx=out:csv&sheet=${encodeURIComponent(sheetName)}`;
          const res = await fetch(url);
          if (!res.ok) throw new Error(`Sheet "${sheetName}" not found or not shared as "Anyone with link can view".`);
          return await res.text();
        };
        
        logSync('Pulling "Cables" tab...');
        const cablesText = await fetchCSV('Cables');
        logSync('Pulling "Ampacity" tab...');
        const ampacityText = await fetchCSV('Ampacity');
        logSync('Pulling "CableOD" tab...');
        const odText = await fetchCSV('CableOD');
        logSync('Pulling "Conduits" tab...');
        const conduitText = await fetchCSV('Conduits');
        logSync('Pulling "Trays" tab...');
        const trayText = await fetchCSV('Trays');
        
        logSync('Parsing sheet data...');
        // Process Cables
        const parsedCables = parseCSV(cablesText);
        const newCableDB = {};
        parsedCables.forEach((row, idx) => {
          if (idx === 0 || row.length < 4 || !row[0]) return;
          const type = row[0];
          const size = row[1];
          const r = row[2];
          const x = row[3];
          if (!newCableDB[type]) newCableDB[type] = [];
          newCableDB[type].push({ size, r, x });
        });
        
        // Process Ampacity
        const parsedAmp = parseCSV(ampacityText);
        const newAmpacityDB = {};
        parsedAmp.forEach((row, idx) => {
          if (idx === 0 || row.length < 5 || !row[0]) return;
          const type = row[0];
          const install = row[1];
          const cores = row[2];
          const size = row[3];
          const amp = Number(row[4]);
          if (!newAmpacityDB[type]) newAmpacityDB[type] = {};
          if (!newAmpacityDB[type][install]) newAmpacityDB[type][install] = {};
          if (!newAmpacityDB[type][install][cores]) newAmpacityDB[type][install][cores] = {};
          newAmpacityDB[type][install][cores][size] = amp;
        });
        
        // Process CableOD
        const parsedOd = parseCSV(odText);
        const newCableOD = {};
        parsedOd.forEach((row, idx) => {
          if (idx === 0 || row.length < 3 || !row[0]) return;
          const size = row[0];
          const cores = row[1];
          const od = Number(row[2]);
          if (!newCableOD[size]) newCableOD[size] = {};
          newCableOD[size][cores] = od;
        });
        
        // Process Conduits
        const parsedConduit = parseCSV(conduitText);
        const newConduitSizes = [];
        parsedConduit.forEach((row, idx) => {
          if (idx === 0 || row.length < 3 || !row[0]) return;
          newConduitSizes.push({
            size: row[0],
            id: Number(row[1]),
            area40: Number(row[2])
          });
        ...
        // Process Trays
        const parsedTray = parseCSV(trayText);
        const newTraySizes = [];
        parsedTray.forEach((row, idx) => {
          if (idx === 0 || row.length < 1 || !row[0]) return;
          const sz = Number(row[0]);
          if (!isNaN(sz) && sz > 0) newTraySizes.push(sz);
        });
        
        setCableDB(newCableDB);
        setAmpacityDB(newAmpacityDB);
        setCableOD(newCableOD);
        if (newConduitSizes.length > 0) setConduitSizes(newConduitSizes);
        if (newTraySizes.length > 0) setTraySizes(newTraySizes);
        
        setSheetStatus('connected');
        logSync('Database successfully synchronized directly from Google Sheet (Read-Only).');
      } else {
        throw new Error('No Google Sheet URL or Apps Script URL set.');
      }
    } catch (e) {
  ...
  const handleSaveProjectToSheet = async () => {
    if (!appsScriptUrl || appsScriptUrl.trim() === '') {
      alert('กรุณาตั้งค่า Google Apps Script Web App URL เพื่อส่งข้อมูลไปบันทึกบน Google Sheet');
      return;
    }
    
    setIsPushing(true);
    logSync('Saving current project to Google Sheet...');
    
    const projectId = currentProjectId || 'PROJ_' + Date.now();
    if (!currentProjectId) {
      setCurrentProjectId(projectId);
      localStorage.setItem('auto_sld_current_project_id', projectId);
    }
    ...
    const projectPayload = {
      ...
      project: {
        projectId,
        projectName: globalSpecs.projectName,
        systemType,
        voltage: calVoltage.toString(),
        pf: globalPf.toString(),
        maxVd: maxVd.toString(),
        globalSpecs,
        feeders
      }
    };
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        logSync(`Project "${globalSpecs.projectName}" successfully saved.`);
        alert('บันทึกโปรเจกต์ลง Google Sheet เรียบร้อยแล้ว!');
        fetchProjectsList();
      } else {
        throw new Error(data.error || 'Failed to save');
      }
    } catch (e) {
      logSync(`Error saving project: ${e.message}`);
  ...
  const handleLoadProjectFromSheet = (proj) => {
    try {
      if (proj.GlobalSpecsJson) {
        const specs = JSON.parse(proj.GlobalSpecsJson);
        setGlobalSpecs(specs);
      }
      if (proj.FeedersJson) {
        const fds = JSON.parse(proj.FeedersJson);
        setFeeders(fds);
      }
      if (proj.SystemType) setSystemType(proj.SystemType);
      if (proj.Voltage) setCalVoltage(Number(proj.Voltage));
      if (proj.Pf) setGlobalPf(Number(proj.Pf));
      if (proj.MaxVd) setMaxVd(Number(proj.MaxVd));
      
      setCurrentProjectId(proj.ProjectID);
      localStorage.setItem('auto_sld_current_project_id', proj.ProjectID);
      
      logSync(`Loaded project: "${proj.ProjectName}"`);
      alert(`โหลดโปรเจกต์ "${proj.ProjectName}" เรียบร้อยแล้ว!`);
    } catch (e) {
      logSync(`Load Error: ${e.message}`);
  ...
  const handleSaveProject = () => {
    const projectData = { globalSpecs, systemType, maxVd, globalPf, calVoltage, feeders, cableOD, conduitSizes, traySizes };
  ...
  const handleExportCSV = () => {
    ...
    feeders.forEach(f => {
      const fR = calculateRaceway(f.cal, systemType, cableOD, conduitSizes, traySizes);
  ...
  const resetCanvasView = () => setCanvasView({ scale: 1, x: 0, y: 0 });
    ...
    <div className="flex h-screen bg-[#0f172a] text-slate-200 overflow-hidden selection:bg-cyan-500 selection:text-white font-sarabun relative">
          ...
          <div className="p-4 space-y-6 overflow-y-auto flex-1 bg-[#1e293b] custom-scrollbar">
              ...
              <div className="space-y-5 animate-in fade-in slide-in-from-left duration-200">
                <section className="border border-[#334155] rounded-xl overflow-hidden shadow-lg bg-[#2d3748]/30">
                  ...
                  <div className="p-0">
                    ...
                    <SidebarTableRow id="trKva" label="Rating (kVA)" value={globalSpecs.trKva} onChange={(v) => updateGlobal('trKva', v)} highlightedField={highlightedField} setHighlightedField={setHighlightedField} inputRef={el => inputRefs.current['trKva'] = el} disabled={globalSpecs.hasTransformer === false} />
            ...
            {activeTab === 'database' && (
              <div className="space-y-3 animate-in fade-in slide-in-from-right duration-200">
                ...
                {['Google Sheet Sync', 'Cable data', 'Raceway', 'EIT Table', 'Transformer', 'Dropout fuse'].map(item => (
                   <button key={item} onClick={() => setDbSubTab(item)} className={`w-full flex items-center gap-3 px-4 py-3.5 rounded-xl text-[13px] font-bold transition-all shadow-md border ${dbSubTab === item ? 'bg-purple-600/20 text-purple-300 border-purple-500/50' : 'bg-[#2d3748] text-slate-400 border-[#334155] hover:bg-[#334155]'}`}>
                      {item === 'Google Sheet Sync' ? (
                        <Cloud className={`w-5 h-5 ${dbSubTab === item ? 'text-purple-400' : 'text-slate-500'}`} />
                      ) : (
      ...
      <div className={`flex-1 overflow-hidden flex justify-center items-start relative ${['database','raceways'].includes(activeTab) ? 'bg-[#0f172a] p-8' : 'bg-[#e2e8f0]'}`}
        ...
        {['database','raceways'].includes(activeTab) ? (
          <div className="w-full max-w-6xl space-y-6 animate-in fade-in zoom-in-95 duration-200 h-full flex flex-col pl-10">
                  ...
                  <table className="w-full text-left border-collapse min-w-[900px]">
                    <thead className="sticky top-0 z-10 shadow-sm">
                      <tr className="bg-[#0f172a] text-slate-300 text-[12px] uppercase tracking-wider font-bold">
                        <th className="py-4 px-4 border-r border-[#334155]">Circuit Name</th>
                        <th className="py-4 px-4 border-r border-[#334155]">Raceway Type</th>
                        <th className="py-4 px-4 border-r border-[#334155]">Cable Configuration</th>
                        <th className="py-4 px-4 border-r border-[#334155] bg-[#1e293b]">Calculated Size ({systemType.includes('3P') ? '3-Phase' : '1-Phase'})</th>
                        <th className="py-4 px-4 border-r border-[#334155] text-amber-400 bg-amber-900/10">Recommended Raceway</th>
                      </tr>
                    </thead>
                      ...
                      {feeders.map(f => {
                         ...
                         return (
                          <tr key={f.id} className="text-[13px] border-b border-[#334155] hover:bg-[#2d3748] transition-colors">
                            <td className="py-4 px-4 font-bold text-slate-200 border-r border-[#334155]">{f.title} <span className="text-slate-500 text-[11px] font-normal block">{f.loadTitle}</span></td>
            ...
            ) : dbSubTab === 'Google Sheet Sync' ? (
              <div className="bg-[#1e293b] rounded-2xl shadow-xl border border-[#334155] overflow-hidden flex-1 flex flex-col min-h-0 text-slate-200">
                <div className="p-6 overflow-y-auto custom-scrollbar h-full space-y-6">
                  
                  {/* Status & Settings Grid */}
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    
                    {/* Connection Form */}
                    <div className="lg:col-span-2 bg-[#0f172a]/40 border border-[#334155] rounded-xl p-5 space-y-4 shadow-inner">
                      <h3 className="text-sm font-bold text-cyan-400 uppercase tracking-wider flex items-center gap-2">
                        <Globe className="w-4 h-4" /> ตั้งค่าการเชื่อมต่อ Google Sheets
                      </h3>
                      
                      <div className="space-y-3">
                        <div>
                          <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
                            Google Sheet URL หรือ ID (แชร์เป็นลิงก์ให้ทุกคนมีสิทธิ์ดู)
                          </label>
                          <input 
                            type="text" 
                      ...
                      <div className="flex gap-3 pt-2">
                        <button 
                          ...
                          disabled={isPulling}
                          className="flex-1 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-bold py-2.5 rounded-lg text-[12px] flex items-center justify-center gap-1.5 shadow-md shadow-cyan-900/40 disabled:opacity-50 transition-all"
                        >
                          <RefreshCw className={`w-3.5 h-3.5 ${isPulling ? 'animate-spin' : ''}`} /> 
                          {isPulling ? 'กำลังดึงข้อมูล...' : 'ซิงค์ดึงข้อมูล (Pull DB)'}
                        </button>
                        
                        <button 
                          onClick={handleSaveProjectToSheet}
                          disabled={isPushing || !appsScriptUrl}
                          className="flex-1 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold py-2.5 rounded-lg text-[12px] flex items-center justify-center gap-1.5 shadow-md shadow-emerald-900/40 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
                        >
                          <UploadCloud className={`w-3.5 h-3.5 ${isPushing ? 'animate-pulse' : ''}`} />
                          บันทึกโปรเจกต์ลงชีต
                        </button>
                      </div>
                    </div>
                    ...
                    {/* Status Display */}
                    <div className="bg-[#0f172a]/40 border border-[#334155] rounded-xl p-5 flex flex-col justify-between shadow-inner">
                      <div>
                        <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider mb-3 flex items-center gap-2">
                          <Server className="w-4 h-4 text-purple-400" /> สถานะการเชื่อมต่อ
                        </h3>
                        
                        <div className="space-y-2">
                          <div className="flex justify-between items-center bg-[#0f172a] px-3 py-2 rounded-lg border border-[#334155]">
                            ...
                            <span className={`text-[11px] font-black px-2.5 py-0.5 rounded-full uppercase ${
                              sheetStatus === 'connected' ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' :
                              sheetStatus === 'syncing' ? 'bg-amber-500/20 text-amber-400 border border-amber-500/30 animate-pulse' :
                              sheetStatus === 'error' ? 'bg-red-500/20 text-red-400 border border-red-500/30' :
                              'bg-slate-700/50 text-slate-400 border border-slate-600'
                            }`}>
                              {sheetStatus === 'connected' ? (appsScriptUrl ? 'Read-Write API' : 'Read-Only CSV') :
                          ...
                          {sheetStatus === 'connected' && (
                            <div className="text-[11px] text-slate-400 space-y-1 bg-[#0f172a]/30 p-2.5 rounded-lg border border-[#334155]/50">
                              <p className="font-bold text-slate-300 flex items-center gap-1">
                                <span className="w-1.5 h-1.5 bg-emerald-400 rounded-full"></span> ฐานข้อมูลที่ใช้งานอยู่ในแอป:
                              </p>
                              <ul className="list-disc pl-4 space-y-0.5 mt-1 font-mono text-[10px] text-slate-400">
                                <li>สายไฟทั้งหมด: {Object.keys(cableDB).length} ประเภท</li>
                                <li>ขนาดกระแส (Ampacity): {Object.keys(ampacityDB['CV(XLPE)-0.6/1kV'] || {}).length} วิธีติดตั้ง</li>
                                <li>ขนาดท่อร้อยสาย: {conduitSizes.length} ขนาด</li>
                                <li>ขนาดรางไฟ: {traySizes.length} ขนาด</li>
                              </ul>
                            </div>
                          )}
                          
                          {sheetError && (
                            <div className="bg-red-950/20 border border-red-500/30 rounded-lg p-2.5 text-[11px] text-red-300 font-mono">
                              <strong>Error:</strong> {sheetError}
                            </div>
                          )}
                        </div>
                      </div>
                      ...
                      <div className="pt-4 text-center">
                        <a 
                          href="https://docs.google.com/spreadsheets/d/1vQYvIuH_F-QJvM4m_82_n5E3n9D3Jc2214V7X-Example/copy" 
                          target="_blank" 
                          rel="noreferrer"
                          className="inline-flex items-center gap-1.5 text-cyan-400 hover:text-cyan-300 text-[11px] font-bold transition-all"
                        >
                          <ExternalLink className="w-3.5 h-3.5" /> คัดลอกเทมเพลต Google Sheet
                        </a>
                      </div>
                    </div>
                  ...
                  {/* Sync Logs & Saved Projects */}
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    
                    {/* Saved Projects List */}
                    <div className="lg:col-span-2 bg-[#0f172a]/20 border border-[#334155] rounded-xl p-5 space-y-4 flex flex-col min-h-[300px]">
                      <h3 className="text-sm font-bold text-emerald-400 uppercase tracking-wider flex items-center gap-2">
                        <FolderOpen className="w-4 h-4" /> โปรเจกต์ที่บันทึกไว้ใน Google Sheet
                      </h3>
                      
                      <div className="flex-1 overflow-auto border border-[#334155] rounded-lg bg-[#0f172a]/30">
                        {savedProjects.length === 0 ? (
                          <div className="py-16 text-center text-slate-500 flex flex-col items-center justify-center">
                            <Cloud className="w-12 h-12 mb-2 opacity-25" />
                            <p className="text-[13px] font-semibold">ไม่มีโปรเจกต์ที่บันทึกไว้</p>
                            <p className="text-[11px] text-slate-600 mt-1">
                              {appsScriptUrl ? 'กดปุ่ม "บันทึกโปรเจกต์ลงชีต" เพื่อเซฟงานชิ้นแรก' : 'โปรดตั้งค่า Apps Script Web App URL เพื่อเปิดใช้งาน'}
                            </p>
                          </div>
                        ) : (
                          <table className="w-full text-left border-collapse text-[12px]">
                            <thead>
                              <tr className="bg-[#0f172a] text-slate-400 font-bold border-b border-[#334155]">
                                <th className="p-3">ชื่อโปรเจกต์</th>
                                <th className="p-3">ระบบไฟฟ้า</th>
                                <th className="p-3">บันทึกเมื่อ</th>
                                <th className="p-3 text-center">การจัดการ</th>
                              </tr>
                            </thead>
                            <tbody>
                              {savedProjects.map((p, idx) => (
                                <tr key={idx} className="border-b border-[#334155] hover:bg-[#2d3748]/50 transition-colors">
                                  <td className="p-3 font-bold text-white">{p.ProjectName}</td>
                                  <td className="p-3 text-slate-300">{p.SystemType} ({p.Voltage}V)</td>
                                  <td className="p-3 text-slate-400 font-mono text-[11px]">{new Date(p.SavedAt).toLocaleString('th-TH')}</td>
                                  <td className="p-3 text-center">
                                    <button 
                                      onClick={() => handleLoadProjectFromSheet(p)}
                                      className="bg-cyan-600 hover:bg-cyan-500 text-white text-[11px] font-bold px-3 py-1 rounded shadow"
                                    >
                                      โหลดแบบ
                                    </button>
                                  </td>
                    ...
                    {/* Sync Logs */}
                    <div className="bg-[#0f172a]/20 border border-[#334155] rounded-xl p-5 space-y-3 flex flex-col h-[300px]">
                      <h3 className="text-sm font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                        <FileText className="w-4 h-4 text-cyan-500" /> ประวัติการซิงค์ข้อมูล (Logs)
                      </h3>
                      
                      <div className="flex-1 bg-[#0f172a] rounded-lg border border-[#334155] p-3 overflow-y-auto font-mono text-[10px] text-cyan-400 space-y-1 custom-scrollbar">
                        {syncLogs.length === 0 ? (
                          <div className="text-slate-600 text-center py-20">ไม่มีประวัติการซิงค์</div>
                        ) : (
                          syncLogs.map((log, idx) => (
                            <div key={idx} className="leading-relaxed border-b border-[#1a202c]/50 pb-1 last:border-0">{log}</div>
                          ))
                        )}
                      </div>
                    </div>
                  ...
                  {/* Setup Tutorial */}
                  <div className="bg-[#0f172a]/20 border border-[#334155] rounded-xl p-6 space-y-4">
                    <h3 className="text-[15px] font-black text-white flex items-center gap-2">
                      <Zap className="w-5 h-5 text-amber-400" /> คู่มือการตั้งค่า Google Sheet & Apps Script
                    </h3>
                    
                    <div className="text-[13px] text-slate-300 space-y-4 leading-relaxed font-sarabun font-normal">
                      <div className="border-l-4 border-cyan-500 pl-4 space-y-1">
                        <p className="font-bold text-cyan-400">ขั้นตอนที่ 1: เตรียม Google Sheet</p>
                        <p>1. กดปุ่มด้านล่างเพื่อคัดลอกไฟล์เทมเพลตชีต</p>
                        <p>2. ใน Google Sheet ให้กดปุ่มแชร์ (Share) ที่มุมขวาบน ปรับการเข้าถึงทั่วไปเป็น <strong>"ทุกคนที่มีลิงก์ (Anyone with link)"</strong> และมีสิทธิ์เป็น <strong>"ผู้มีสิทธิ์อ่าน (Viewer)"</strong></p>
                        <p>3. คัดลอกลิงก์หรือ Spreadsheet ID มาวางในช่องตั้งค่าด้านบน เพื่อดึงฐานข้อมูลแบบ Read-Only</p>
                      </div>
                      
                      <div className="border-l-4 border-amber-500 pl-4 space-y-2">
                        <p className="font-bold text-amber-400">ขั้นตอนที่ 2: ตั้งค่า Apps Script (สำหรับบันทึก/โหลดโปรเจกต์)</p>
                        <p>1. ใน Google Sheet ของคุณ ไปที่เมนู <strong>ส่วนขยาย (Extensions) &gt; Apps Script</strong></p>
                        <p>2. คัดลอกโค้ดจากปุ่ม <strong>"คัดลอกโค้ด Apps Script"</strong> ด้านล่างนี้ ไปวางทับโค้ดทั้งหมดในหน้าต่าง Apps Script แล้วกดปุ่มบันทึก (แผ่นดิสก์)</p>
                        <p>3. กดปุ่ม <strong>การทำให้ใช้งานได้ (Deploy) &gt; การทำให้ใช้งานได้ใหม่ (New deployment)</strong></p>
                        <p>4. เลือกประเภทเป็น <strong>"เว็บแอป (Web app)"</strong> ตั้งค่าดังนี้:</p>
                        <ul className="list-disc pl-6 space-y-1 text-slate-400 text-[12px]">
                          <li>เรียกใช้งานในฐานะ (Execute as): <strong>"ฉัน (Me)"</strong></li>
                          <li>ผู้มีสิทธิ์เข้าถึง (Who has access): <strong>"ทุกคน (Anyone)"</strong> <span className="text-red-400 font-bold">*สำคัญมาก ห้ามเลือกเป็น Anyone with Google account</span></li>
                        </ul>
                        <p>5. กด Deploy และคัดลอก <strong>URL เว็บแอป (Web app URL)</strong> ที่ได้ นำมาวางในช่องตั้งค่าด้านบน</p>
                      </div>
                      
                      <div className="pt-2 flex justify-start">
                        <button 
                          onClick={() => {
                            // Fetch standard AppsScriptCode
                            const codeUrl = 'AppsScriptCode.js';
                            fetch(codeUrl)
                              .then(r => r.text())
                              .then(txt => {
                                const el = document.createElement('textarea');
                                ...
                                document.execCommand('copy');
                                document.body.removeChild(el);
                                alert('คัดลอกโค้ด Apps Script ไปยัง Clipboard แล้ว! คุณสามารถนำไปกด Paste ใน Google Apps Script ได้เลย');
                              })
                              .catch(() => {
                                alert('ไม่สามารถโหลดโค้ด Apps Script ได้โดยตรง กรุณาคัดลอกไฟล์ AppsScriptCode.js ในโฟลเดอร์ของระบบ');
                              });
                          }}
                          className="bg-[#2d3748] hover:bg-[#334155] border border-[#475569] text-cyan-400 font-bold px-5 py-2.5 rounded-xl text-[12px] transition-all flex items-center gap-1.5 shadow-inner"
                        >
                          <Copy className="w-4 h-4" /> คัดลอกโค้ด Apps Script
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-md overflow-hidden border border-slate-300 shadow-2xl relative" style={{ transformOrigin: 'top left', transform: `translate(${canvasView.x}px, ${canvasView.y}px) scale(${canvasView.scale})`, transition: isDragging ? 'none' : 'transform 0.1s ease-out' }}>
              <svg ref={svgRef} viewBox={`0 0 ${svgWidth} 1000`} width={svgWidth} height={1000} className="block relative z-10" style={{ backgroundColor: 'white' }}>
                ...
                <g>
                  {globalSpecs.hasTransformer !== false ? (
                    <g>
                      <circle cx={centerX} cy="100" r="35" fill="white" stroke="#1e293b" strokeWidth="3" />
                      <circle cx={centerX} cy="145" r="35" fill="white" stroke="#1e293b" strokeWidth="3" />
                      <line x1={centerX} y1="180" x2={centerX} y2={feeders.length === 1 ? 480 : 280} stroke="#1e293b" strokeWidth="3" />
                    </g>
                  ) : (
                    <g>
                      <line x1={centerX} y1="120" x2={centerX} y2={feeders.length === 1 ? 480 : 280} stroke="#1e293b" strokeWidth="3" />
                      <polygon points={`${centerX},100 ${centerX - 8},120 ${centerX + 8},120`} fill="#1e293b" />
                    </g>
                  )}
                  
                  <rect x={centerX - (renderItems.length * feederWidth / 2) - 60} y="220" width={(renderItems.length * feederWidth) + 120} height="400" fill="transparent" stroke="#94a3b8" strokeWidth="2" strokeDasharray="12,6" rx="8" />
                  ...
                  {renderItems.map((item, i) => { 
                    ...
                    if (item.type === 'skip') {
                      return (
                        <g key={`l-skip-${i}`}>
                          <line x1={xPos} y1="420" x2={xPos} y2="700" stroke="#94a3b8" strokeWidth="3" strokeDasharray="8,6" />
                        </g>
                      );
                    }
                    const f = item.feeder;
                    return (
                      <g key={`l-${f.id}`}>
                        ...
                        <line x1={xPos} y1="515" x2={xPos} y2="700" stroke="#1e293b" strokeWidth="3" />
                      </g>
                    ) 
                  })}
                </g>
                    ...
                    return (
                      <g key={`c-${f.id}`}>
                        ...
                        <g cursor="pointer" onClick={() => focusInput('feeders', `f-${f.id}-loadKw`)} style={{ cursor: 'pointer' }}>
                          <rect x={xPos - 55} y={700} width="110" height="65" fill="#f8fafc" stroke="#334155" strokeWidth="2.5" rx="6" />
                          <path d={`M ${xPos - 55} 720 L ${xPos + 55} 720`} stroke="#cbd5e1" strokeWidth="1" />
                          <text x={xPos} y={715} textAnchor="middle" fontSize="11" fill="#1e293b" style={{ fontWeight: 'bold' }}>{f.loadTitle}</text>
                          <text x={xPos} y={738} textAnchor="middle" fontSize="14" fill="#0369a1" style={{ fontWeight: 'bold', fontFamily: 'Sarabun, monospace' }}>{f.loadKw} kW</text>
                          <text x={xPos} y={753} textAnchor="middle" fontSize="11" fill="#64748b" style={{ fontFamily: 'Sarabun, monospace' }}>({f.loadA} A)</text>
                        </g>
                      </g>
                    ); 
                  })}
                ...
                <g className="labels">
                  {globalSpecs.hasTransformer !== false ? (
                    <g transform={`translate(${centerX + 60}, 60)`} cursor="pointer" onClick={() => focusInput('global', 'trTitle')}>
                      <rect x="-10" y="-20" width="280" height="110" fill="white" fillOpacity="0.8" rx="4" />
                      {[
                        ...
                        { val: globalSpecs.trVector, fs: 12, c: '#475569', b: false }, 
                        { val: globalSpecs.trZk, fs: 12, c: '#475569', b: false }
                    ...
                    <g transform={`translate(${centerX}, 85)`} cursor="pointer" onClick={() => focusInput('global', 'existingMdbText')}>
                  ...
                  {feeders.length > 1 && (
                    <g transform={`translate(${centerX + 15}, 200)`} cursor="pointer" onClick={() => focusInput('global', 'mainPhaseCable')}>
                  ...
                  {renderItems.map((item, i) => { 
                    ...
                    return (
                      <g key={`fl-${f.id}`} cursor="pointer" onClick={() => focusInput('feeders', `f-${f.id}-title`)}>
                        ...
                        <g transform={`rotate(-90, ${xPos - 12}, 650)`} onClick={(e) => { e.stopPropagation(); focusInput('feeders', `f-${f.id}-cable1`); }}>
                          <text x={xPos - 12} y="635" fontSize="11" fill="#1e293b" style={{ fontWeight: 'bold', fontFamily: 'Sarabun, monospace', textTransform: 'uppercase' }}>{f.cable1}</text>
                          <text x={xPos - 12} y="650" fontSize="11" fill="#047857" style={{ fontWeight: 'bold', fontFamily: 'Sarabun, monospace', textTransform: 'uppercase' }}>{f.cable2}</text>
                          <text x={xPos - 12} y="680" fontSize="10" fill="#64748b" style={{ fontStyle: 'italic', fontFamily: 'Sarabun, monospace' }}>{f.install}</text>
                        </g>
                      </g>
                    ); 
                  })}
