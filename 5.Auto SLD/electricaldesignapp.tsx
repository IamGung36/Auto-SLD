  ...
  const fetchProjectsList = async (scriptUrl = appsScriptUrl) => {
    ...
    try {
      ...
      const errMsg = e.message === 'Failed to fetch'
        ? 'Failed to fetch (อาจเกิดจากสิทธิ์เข้าถึง Web App ใน Apps Script ไม่ถูกต้อง หรือยังไม่ได้ Deploy)'
        : e.message;
      logSync(`Failed to fetch project list: ${errMsg}`);
    }
  };
  ...
  const handleSaveProjectToSheet = async () => {
    ...
    try {
      ...
      const errMsg = e.message === 'Failed to fetch'
        ...
        : e.message;
      logSync(`Error saving project: ${errMsg}`);
  ...
  const handleLoadProjectFromSheet = (proj) => {
    try {
      if (proj.GlobalSpecsJson) {
        ...
        if (specs.racewayGroups) {
          setRacewayGroups(specs.racewayGroups);
        } else {
          setRacewayGroups([
            { id: 1, startNum: 1, endNum: 4 },
            { id: 2, startNum: 5, endNum: 7 }
